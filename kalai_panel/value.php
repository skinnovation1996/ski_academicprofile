...
<head>
	...
	<title>Value - Academic Profile <?php echo $user_role;?></title>
...
<div class="wrapper">
    ...
    -->
    	...
    	<div class="sidebar-wrapper">
            ...
            <?php $navactive = 12; include("assets/php/Admin-navbar.php");?>
    	</div>
    </div>
        ...
        <nav class="navbar navbar-default navbar-fixed">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="value.php">Value</a>
                </div>
                <div class="collapse navbar-collapse">
                    <ul class="nav navbar-nav navbar-left">
                        <li>
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
								<p class="hidden-lg hidden-md">Value</p>
                            </a>
                        </li>
                    </ul>
                            ...
                            <div class="content table-responsive">
                                ...
                                <table id="tlist" class="table table-hover table-striped">
                                    <thead>
                                    	...
                                    	<th width="75%" style="text-align:center">Title</th>
                                        <th width="10%" style="text-align:center">Date &amp; Time</th>
                                        <th width="10%" style="text-align:center">Actions</th>
                                    </thead>
                                    <tbody>
                                    <button type= "button" data-toggle="modal" data-target="#AddData" class="add-data btn btn-success btn-sm">+ Add New Value</button>
                                        <?php
                                        $sql = mysqli_query($conn, "SELECT * from tbl_value WHERE super_owner='$login_super_owner' ORDER BY value_date DESC");
                                        $rows = mysqli_num_rows($sql);
                                              
                                        if($rows){
                                            $count = 1;
                                            while($row=mysqli_fetch_array($sql)){
                                                $datetimes = new DateTime($row['value_date']);
                                                $date_sel = date_format($datetimes, "Y-m-d");
                                                $time_sel = date_format($datetimes, "H:i:s");
                                        ?>
                                        <tr>
                                            <td><?php echo $count++;?></td>
                                            <td><?php 
                                            if($row['value_file'] != NULL){
                                            ?><a href="uploads/values/<?php echo $row['value_file'];?>" target="_new"><b><?php echo $row['value_title'];?></b></a>
                                            <?php
                                            }else{
                                                echo "<b>".$row['value_title']."</b>";
                                            }?></td>
                                            <td><?php echo $row['value_date'];?></td>
                                            <td><button type="button" data-toggle="modal" data-id="<?php echo $row['id'];?>" data-title="<?php echo $row['value_title'];?>" data-date="<?php echo $date_sel;?>" data-time="<?php echo $time_sel;?>"
                                            data-content="<?php echo $row['value_content'];?>" data-file="<?php echo $row['value_file'];?>" data-target="#EditData" class="edit-data btn btn-primary btn-sm">EDIT</button>
...
<div class="modal fade" id="AddData" role="dialog">
	<div class="modal-dialog">
    	...
    	<!-- Modal content-->
      	<div class="modal-content">
		  	<form id="adddata" name="adddata" method="post" enctype="multipart/form-data" action="operations/add-value.php">
        		<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">Add Value</h4>
                    <p class="category">For each value added, 5 credits (10 credits with file) will be deducted from your credit balance.</p>
				</div>
				<div class="modal-body">
                    <div class="row">
                        <div class="col-md-12">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label>Content</label>
                                <textarea name="content" id="content" class="form-control" placeholder="Content..." required columns="3" rows="3"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="file" name="valuefile" id="valuefile">
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="EditData" role="dialog">
	<div class="modal-dialog">
    	...
    	<!-- Modal content-->
      	<div class="modal-content">
		  	<form id="editdata" name="editdata" method="post" enctype="multipart/form-data" action="operations/edit-value.php">
        		<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
...
<!-- Confirm Delete Modal -->
  <div class="modal fade" id="ConfirmDelete" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <form action="operations/delete-value.php" method="post" class="form-horizontal" enctype="multipart/form-data">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Are You Sure?</h4>
        </div>
        <div class="modal-body">
          <p>WARNING! Are you sure to delete this value?</p>
          <input type="hidden" name="oldid" id="oldid" value=""/>
        </div>
        <div class="modal-footer">
	...
	<script type="text/javascript">
    	$(document).ready(function(){
        	...
        	$(document).on("click", ".edit-data", function () {
                ...
                $(".modal-body #content").val( dataContent );
                $(".modal-body #date").val( dataDate );
                $(".modal-body #time").val( dataTime );
