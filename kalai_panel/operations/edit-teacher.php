...
if(isset($_POST['submit-button'])){
    ...
    if($tmpprofile_pic != NULL){
        ...
        if(file_exists("../uploads/admins/$user_id/" . basename($profile_pic))){
            ...
            header("location:../edit-teacher.php?id=$sqlgetid");
        }
        ...
        if (move_uploaded_file($tmpprofile_pic, "../uploads/admins/$user_id/" . basename($profile_pic))) {
            ...
            $sql = mysqli_query($conn, "UPDATE tbl_admin SET profile_pic='$file_uploaded1' WHERE id='$sqlgetid' admin_id='$user_id' AND role='Teacher'");
            if($sql === false){
    ...
    if($tmpfront_pic != NULL){
        ...
        if (move_uploaded_file($tmpfront_pic, "../uploads/admins/$user_id/" . basename($front_pic))) {
            ...
            $sql = mysqli_query($conn, "UPDATE tbl_admin SET front_pic='$file_uploaded2' WHERE id='$sqlgetid' AND admin_id='$user_id' AND role='Teacher'");
            if($sql === false){
                $errorCode = "SQL_DB_FAILED";
                $errorMsg = "There's a problem with MySQL Database. Please contact administrator.<br>Error Details: ". mysqli_error();
                $_SESSION['academicprofile_error_msg'] = $errorMsg . " (Error Code: $errorCode)";
    ...
    if($errorCode == NULL){
        ...
        WHERE id='$sqlgetid' AND admin_id='$user_id' AND role='Teacher'";
    ...
    if($errorCode == NULL){
        $_SESSION['academicprofile_success_msg'] = "You have successfully edited the teacher's profile!";
