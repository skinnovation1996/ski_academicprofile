...
if(isset($_POST['submit-button'])){
    ...
    $role = mysqli_real_escape_string($conn,"Teacher");
    ...
    if($errorCode == NULL){
        ...
        $query = "INSERT INTO tbl_admin (admin_id, admin_name, role, tagline, department, 
        university, city, postcode, education_level, tel, fax, country, specializations, 
        email, facebook, instagram, linkedin, twitter, location, credits, plan, profile_pic, front_pic, admin_password) 
        VALUES('$user_id','$name','$role','$tagline','$department','$university','$city','$postcode','$academic_level'
        '$tel','$fax','$country','$specializations','$email','$facebook','$instagram',
        '$linkedin','$twitter', 'Coming soon', '$credits','$plan','$file_uploaded1','$file_uploaded2','$password')";
    ...
    if($errorCode == NULL){
        //Reserved for registration email sent to user

        $_SESSION['academicprofile_success_msg'] = "You have successfully added the teacher! An email has been sent to the teacher for logging in to the system.";
